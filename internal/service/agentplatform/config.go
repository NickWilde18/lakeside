...
func loadConfig(ctx context.Context) (*config, error) {
	...
	cfg.Storage.Cleanup.Enabled = g.Cfg().MustGet(ctx, "agents.storage.cleanup.enabled", true).Bool()
	cfg.Storage.Cleanup.IntervalMinutes = g.Cfg().MustGet(ctx, "agents.storage.cleanup.intervalMinutes", 1440).Int()
	cfg.Storage.Cleanup.DeletedRetentionHours = g.Cfg().MustGet(ctx, "agents.storage.cleanup.deletedRetentionHours", 168).Int()
	cfg.Storage.Cleanup.BatchSize = g.Cfg().MustGet(ctx, "agents.storage.cleanup.batchSize", 100).Int()
	...
	cfg.DeerFlow.BaseURL = strings.TrimSpace(g.Cfg().MustGet(ctx, "agents.deerflow.baseURL").String())
	cfg.DeerFlow.AssistantID = strings.TrimSpace(g.Cfg().MustGet(ctx, "agents.deerflow.assistantID", "lead_agent").String())
	cfg.DeerFlow.TimeoutMs = g.Cfg().MustGet(ctx, "agents.deerflow.timeoutMs", 300000).Int()
	cfg.DeerFlow.DefaultModel = strings.TrimSpace(g.Cfg().MustGet(ctx, "agents.deerflow.defaultModel").String())
	cfg.DeerFlow.ThinkingEnabled = g.Cfg().MustGet(ctx, "agents.deerflow.thinkingEnabled", false).Bool()
	cfg.DeerFlow.PlanMode = g.Cfg().MustGet(ctx, "agents.deerflow.planMode", true).Bool()
